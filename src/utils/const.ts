...
const CHINESE_INFO_MESSAGE = (yearLength: number, year: string): string => {
  ...
  return `用运动 App 记录自己的运动 ${yearLength} 年了，下面列表展示的是${yearStr}的数据`;
};
const ENGLISH_INFO_MESSAGE = (yearLength: number, year: string): string =>
  `Tracking my sports for ${yearLength} years. The table below shows the ${yearStr} data.`;

// English is not supported for location info messages yet
const CHINESE_LOCATION_INFO_MESSAGE_FIRST =
  '记录了一些轨迹，希望随着时间推移，点亮的地方越来越多';
const CHINESE_LOCATION_INFO_MESSAGE_SECOND = '保持热爱，继续记录';
...
export const WALKING_COLOR = 'rgb(0,200,200)';
...
export const MAP_TILE_ACCESS_TOKEN = '8lAomZ2Nlw9iCBS1nowf';
